...
#----------REALIZANDO DETECCIÓN DE GRIETAS------------
# Importando la imagen
img = cv.imread('C:/Users/USUARIO/Documents/Segmentation/afa.jpg')
...
# Aplicando detección de círculos de Hough. Ajustar parámetros para intentar detectar un solo círculo de tamaño correcto. Intenta ajustar param1 y param2 primero
...
if circles is not None:
    ...
    # Poniendo texto en la imagen mostrando el diámetro
    cv.putText(img, f"Diámetro: {diameter}px", (10, 30), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv.LINE_AA)

    # Convirtiendo la imagen a RGB para mostrarla con matplotlib
    img_rgb = cv.cvtColor(img, cv.COLOR_BGR2RGB)

    # Mostrando la imagen con matplotlib
    plt.imshow(img_rgb)
    plt.title('Círculo Detectado con Diámetro')
    plt.axis('off')
    plt.show()
else:
